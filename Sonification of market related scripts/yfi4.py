...
def get_root_note_from_price(price):
    base = 48  # Do3
    step = int((price - min_price) / (max_price - min_price) * 12)
    return base + step

def build_chord(root, is_major=True):
    if is_major:
...
for index, row in df.iterrows():
    root = get_root_note_from_price(row["Close"]) + pitch_shift
    ...
    chord = build_chord(root, is_major(row))
